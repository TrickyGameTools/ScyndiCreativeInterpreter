...
#pragma region "Only used for savegame. Headers are in API_SGJCR.cpp"
	Party JSG_GetParty(string T,bool create) {
		...
		if (!PartyReg.count(T)) Crash("SG:There is no party tagged '"+T+"' in the statistician database");
		return PartyReg[T];
	}
...
//}
	...
	static int API_HaveChar(lua_State *L) {
        auto
            PartyTag{ Upper(Lunatic_CheckString(L,1)) },
            CharTag{ Lunatic_CheckString(L,2) };
        if (!PartyReg.count(PartyTag)) Crash(TrSPrintF("There is no party tagged '%s' in the statistician database"));
	...
	static int API_CharListOfStats(lua_State*L) {
		BaseTag;
		auto idx{luaL_checkinteger(L,3)};
		static VecString _SL{nullptr};
		if (idx<0) {
			_SL=CharData->StatList();
			lua_pushinteger(L,_SL->size());
			return 1;
		}
		Lunatic_PushString(L,(*_SL)[idx]);
		return 1;
	}

	static int API_CharList(lua_State*L){
		auto PartyTag{Upper(Lunatic_CheckString(L,1))};
		auto id{luaL_checkinteger(L,2)}; if (!PartyReg.count(PartyTag)) { luaL_error(L,"CharList(%d) There is no party tagged '%s' in the statistician database",id,PartyTag.c_str()); return 0; }
		static VecString ChList{nullptr};
		if (id<0) {
			ChList = PartyReg[PartyTag]->CharList();
			lua_pushinteger(L,ChList->size());
			return 1;
		}
		if (!ChList) { luaL_error(L,"PartyReg[\"%s\".CharList(%d): Character list not yet retrieved!",PartyTag.c_str(),id); return 0; }
		if (id>=ChList->size()) { luaL_error(L,"PartyReg[\"%s\".CharList(%d): Character list index out of bounds (%d)",PartyTag.c_str(),id),ChList->size(); return 0; }
		Lunatic_PushString(L,(*ChList)[id]);
		return 1;
	}
...
API_Link(API_LinkPnts,LinkPoints);
	...
	void Init_API_Statistician() {
		std::map<std::string, lua_CFunction>IAPI{
			...
			{ "ListOfStats",API_CharListOfStats	},
			{ "CharList", API_CharList }
		};
